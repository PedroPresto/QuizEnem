...
@WebServlet("/getIdsQuestoesSubMaterias")
public class GetIdsQuestoesSubMateriasServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBConnection.getConnection()) {
            for (MateriaQuantidade item : materias) {
                ...
                boolean isAnoAleatorio = true;

                if (isAnoAleatorio) {
                    // Se o ano for "aleatorio", a SQL não filtra por ano
                    sql = "SELECT id FROM questoes WHERE id_sub_materia = ?";
                } else {
                    ...
                    sql = "SELECT q.id FROM questoes q JOIN ano a ON q.id_ano = a.id WHERE q.id_sub_materia = ? AND a.ano = ?";
                }
